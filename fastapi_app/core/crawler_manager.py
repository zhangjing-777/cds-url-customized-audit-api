...
from ..database import CrawlerTask
...
class CrawlerManager:
    ...
    def start_crawler(self, site_name: str, db: Session):
        ...
        try:
            ...
            logger.error(f"Database error in start_crawler: {e}")
            db.rollback()
            return {"success": False, "message": "Database error occurred"}
        except Exception as e:
            logger.error(f"Unexpected error in start_crawler: {e}")
            db.rollback()
    ...
    def stop_crawler(self, site_name: str, db: Session):
        try:
            ...
            logger.error(f"Database error in stop_crawler: {e}")
            db.rollback()
            return {"success": False, "message": "Database error occurred"}
        except Exception as e:
            logger.error(f"Unexpected error in stop_crawler: {e}")
            return {"success": False, "message": "An unexpected error occurred"}
